...
#Filter out the single sites
COI <- COI[,colnames(COI)!="L.PE.B" & colnames(COI)!="L.DN1"]
#Filter out the freshwater lagoon & the replicate Saldhana bay site
COI <- COI[,-grep("DC|SB",colnames(COI))]
...
#Filter out the single sites
zhan <- zhan[,colnames(zhan)!="Z.PE.B" & colnames(zhan)!="Z.DN1"]
#Filter out the freshwater lagoon & the replicate Saldhana bay site
zhan <- zhan[,-grep("DC|SB",colnames(zhan))]
...
#Filter out the single sites
ProK <- ProK[,colnames(ProK)!="PE.B" & colnames(ProK)!="DN1"]
#Filter out the freshwater lagoon & the replicate Saldhana bay site
ProK <- ProK[,-grep("DC|SB",colnames(ProK))]
...
colnames(ProKcontrols) <- colnames(ProKtaxa)
temp <- as.data.frame(apply(ProKtaxa,2, as.character),stringsAsFactors=FALSE)
for (row in 1:dim(controls)[1]){
  if(!rownames(controls)[row] %in% ProKtaxa$OTUname){
    next()}
  ProKcontrols[rownames(ProKcontrols)[row],] <-  temp[na.omit(match(rownames(ProKcontrols)[row],as.character(temp$OTUname))),]
}
...
##All toghehr
pdf("figures/map.test.pdf",width=11,height=6)
m <- rbind(c(2,1,4),c(2,3,3))
layout(m)
par(mar = c(3, 3, 0, 0))
map("world", xlim=c(14,34), ylim=c(-36,-26), col="gray", fill=TRUE)
map.axes()
map('worldHires', xlim=c(17.5,19), ylim=c(-34.5,-32.5), col="gray", fill=TRUE,resolution=0)
map.axes()
map('worldHires', xlim=c(21.5,28.5), ylim=c(-34.5,-32.5), col="gray", fill=TRUE,resolution=0)
map.axes()
map('worldHires', xlim=c(30.5,32.5), ylim=c(-30.5,-28.5), col="gray", fill=TRUE,resolution=0)
map.axes()
dev.off()


(west <- ggplot(data = worldHR) +
    geom_sf(fill = "cornsilk") +
    coord_sf(xlim = c(17.5,19), ylim = c(-34.5,-32.5), expand = FALSE,datum=NA))

worldHR <- ne_countries(scale='large',returnclass = 'sf')


ggplot(data = world) +
  geom_sf(colour="black",fill="grey")+
  coord_sf(xlim = c(17.5,19), ylim = c(-34.5,-32.5), expand = FALSE,datum=NA)
...
summary(ANOVA.18S) #Global P =0.196
...
pdf("figures/OTU.coast.Richness.pdf",width = 7,height=5)

plot(jitter(as.numeric(Coastal.alpha.tall$comb),0.8),
     Coastal.alpha.tall$value,pch=16,col=as.numeric(Coastal.alpha.tall$variable),
     xaxt="n",xlab="Coast",
     ylab="OTUs")
axis(1,at=c(2,5,8),labels=c("West","South","East"),cex=0.8)
...
legend(22,1,rownames(COI.taxa.count),col=getPalette(dim(COI.taxa.count)[1]),pch=16,cex=0.6)
...
legend(22,1,rownames(z18S.taxa.count),col=getPalette(dim(z18S.taxa.count)[1]),pch=16,cex=0.6)
barplot(prop.table(as.matrix(ProK.taxa.count[,na.omit(match(latlongdata$sitecode,colnames(ProK.taxa.count)))]),2),col=getPalette(dim(ProK.taxa.count)[1]),axisnames=FALSE)
legend(22,1,rownames(ProK.taxa.count),col=getPalette(dim(ProK.taxa.count)[1]),pch=16,cex=0.6)
dev.off()
...
legend(22,0.4,rownames(COI.taxa.count)[-1],col=getPalette(dim(COI.taxa.count)[1]-1),pch=16,cex=0.6)
...
legend(22,0.75,rownames(z18S.taxa.count)[-1],col=getPalette(dim(z18S.taxa.count)[1]-1),pch=16,cex=0.6)
barplot(prop.table(as.matrix(ProK.taxa.count[,na.omit(match(latlongdata$sitecode,colnames(ProK.taxa.count)))]),2)[-1,],col=getPalette(dim(ProK.taxa.count)[1]),axisnames=FALSE,main="16S")
legend(22,1,rownames(ProK.taxa.count)[-1],col=getPalette(dim(ProK.taxa.count)[1]-1),pch=16,cex=0.6)
dev.off()
...
legend(22,1,rownames(COI.taxa.abun),col=getPalette(dim(COI.taxa.abun)[1]),pch=16,cex=0.6)
...
legend(22,1,rownames(z18S.taxa.abun),col=getPalette(dim(z18S.taxa.abun)[1]),pch=16,cex=0.6)
barplot(prop.table(as.matrix(ProK.taxa.abun[,na.omit(match(latlongdata$sitecode,colnames(ProK.taxa.abun)))]),2),col=getPalette(dim(ProK.taxa.count)[1]),axisnames=FALSE)
legend(22,1,rownames(ProK.taxa.abun),col=getPalette(dim(ProK.taxa.abun)[1]),pch=16,cex=0.6)
dev.off()

pdf("figures/Figure1/AbundanceTab.noUnassign.pdf",width = 11, height = 7)
par(mfrow=c(3,1),mai = c(0.3, 0.5, 0.1,1),xpd=TRUE)
barplot(prop.table(as.matrix(COI.taxa.abun[,na.omit(match(latlongdata$sitecode,colnames(COI.taxa.abun)))]),2)[-1,],col=getPalette(dim(COI.taxa.count)[1]),axisnames=FALSE)
legend(22,0.8,rownames(COI.taxa.abun)[-1],col=getPalette(dim(COI.taxa.abun)[1]-1),pch=16,cex=0.6)
barplot(prop.table(as.matrix(z18S.taxa.abun[,na.omit(match(latlongdata$sitecode,colnames(z18S.taxa.abun)))]),2)[-1,],col=getPalette(dim(z18S.taxa.count)[1]),axisnames=FALSE)
legend(22,1,rownames(z18S.taxa.abun)[-1],col=getPalette(dim(z18S.taxa.abun)[1]-1),pch=16,cex=0.6)
barplot(prop.table(as.matrix(ProK.taxa.abun[,na.omit(match(latlongdata$sitecode,colnames(ProK.taxa.abun)))]),2)[-1,],col=getPalette(dim(ProK.taxa.count)[1]),axisnames=FALSE)
legend(22,1,rownames(ProK.taxa.abun)[-1],col=getPalette(dim(ProK.taxa.abun)[1]-1),pch=16,cex=0.6)
dev.off()
...
#Plot
pdf("figures/COI.beta.pdf",width = 9,height=5.5)
par(mfrow=c(1,2))
plot(nMDS$points,col=latlongdata$PERMori[match(rownames(nMDS$points),latlongdata$sitecode)],pch=16,main="COI - nMDS - Jaccard",cex=3)
text(nMDS$points[,1],nMDS$points[,2],labels=rownames(nMDS$points))

plot(pcoa,col=latlongdata$PERMori[match(rownames(pcoa),latlongdata$sitecode)],pch=16,cex=3,main="COI - PCoA - Jaccard")
text(pcoa[,1],pcoa[,2],labels=rownames(pcoa))
dev.off()

#What about plotting the relationship between temperature and nMDS axes?
pdf("figures/COI.temp.nMDS.pdf",width = 5,height=5)
plot(tempdat$TempAvr[match(names(nMDS$points[,1]),tempdat$Site.Code)],nMDS$points[,1],pch=16,xlab="2-year Mean Temp (C)",ylab="nMDS Axis1",main="COI")
...
#Plot
pdf("figures/r18S.beta.pdf",width = 9,height=5.5)
par(mfrow=c(1,2))
plot(nMDS$points,col=latlongdata$PERMori[match(rownames(nMDS$points),latlongdata$sitecode)],pch=16,main="18S - nMDS - Jaccard",cex=3)
text(nMDS$points[,1],nMDS$points[,2],labels=rownames(nMDS$points))

plot(pcoa,col=latlongdata$PERMori[match(rownames(pcoa),latlongdata$sitecode)],pch=16,cex=3,main="18S - PCoA - Jaccard")
text(pcoa[,1],pcoa[,2],labels=rownames(pcoa))
dev.off()

#What about plotting the relationship between temperature and nMDS axes?
pdf("figures/18S.temp.nMDS.pdf",width = 5,height=5)
plot(tempdat$TempAvr[match(names(nMDS$points[,1]),tempdat$Site.Code)],nMDS$points[,1],pch=16,xlab="2-year Mean Temp (C)",ylab="nMDS Axis1",main="18S")
abline(lm(nMDS$points[,1]~tempdat$TempAvr[match(names(nMDS$points[,1]),tempdat$Site.Code)]),col="red")
dev.off()
...
#Plot
pdf("figures/ProK.beta.pdf",width = 9,height=5.5)
par(mfrow=c(1,2))
plot(nMDS$points,col=latlongdata$PERMori[match(rownames(nMDS$points),latlongdata$sitecode)],pch=16,main="16S - nMDS - Jaccard",cex=3)
text(nMDS$points[,1],nMDS$points[,2],labels=rownames(nMDS$points))

plot(pcoa,col=latlongdata$PERMori[match(rownames(pcoa),latlongdata$sitecode)],pch=16,cex=3,main="16S - PCoA - Jaccard")
text(pcoa[,1],pcoa[,2],labels=rownames(pcoa))
dev.off()

#What about plotting the relationship between temperature and nMDS axes?
pdf("figures/16S.temp.nMDS.pdf",width = 5,height=5)
plot(tempdat$TempAvr[match(names(nMDS$points[,1]),tempdat$Site.Code)],nMDS$points[,1],pch=16,xlab="2-year Mean Temp (C)",ylab="nMDS Axis1",main="16S")
abline(lm(nMDS$points[,1]~tempdat$TempAvr[match(names(nMDS$points[,1]),tempdat$Site.Code)]),col="red")
dev.off()
...
legend("topright",col=1:2,legend = levels(as.factor(pairwise.nomixed$type)),pch=16)
x <-Kmdatapair$value[match(pairwise.nomixed$comp,Kmdatapair$comp)]
y <- log10(1-pairwise.nomixed$value)
abline(lm(y[pairwise.nomixed$type=="Artificial"] ~ x[pairwise.nomixed$type=="Artificial"]),lwd=2,col=1)
abline(lm(y[pairwise.nomixed$type=="Natural"] ~ x[pairwise.nomixed$type=="Natural"]),lwd=2,col=2)
dev.off()
...
HeaderOut <- c("ID","Nobs","Pperm.coast","Pdisp","DistDecayPoverall","DistDecayPsiteadd","DistDecayPsiteInt")
